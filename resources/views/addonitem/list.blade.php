          ...
          <div class="col-md-12">
               <div class="card">
                    <div class="card-header">
                         <h4 class="card-title" style="padding-left:20px;">Addon Items</h4>
                    </div>
                    <div class="card-body">
                         <div class="bootstrap-table">
                              <div class="fixed-table-toolbar">
                                   ...
                                   <div class="columns columns-right pull-right mt-0" >
                                        <a href="{{ url('addonitem/create')}}?{{$request->getQueryString() }}" style="font-size:16px;padding: 7px 20px;"
                                   ...
                                   <div class="pull-left search">
                                        <form method="get" action="" class="form-inline">
                                             ...
                                             <div class="form-group" style="padding-right: 5px;margin-bottom:0px;">
                                                  ...
                                                  <select name="addon_categories_id" class="form-control" id="sel1">
                                                       <option value="">Addon Category</option>
                                                       @foreach($addon_cat as $acat)
                                                       <option value="{{ $acat->id }}"
                                                            {{ Request()->addon_categories_id == $acat->id ? 'selected' : '' }}>
                                                            {{ $acat->name }}</option>
                                                       @endforeach
                                                  </select>
                                             </div>
                                             ...
                                             <table id="bootstrap-table" class="table table-hover table-sm "
                                                  ...
                                                  <tbody>
                                                       ...
                                                       <tr>
                                                            ...
                                                            <td style="">{{ \Illuminate\Support\Str::limit($item->addon_category, 30) }}</td>
                                                            <td style="">{{ \Illuminate\Support\Str::limit($item->description, 30) }}</td>
                                                            <td class="text-center">{{ $item->sort_order }}</td>
                                                            <td class="text-center">
                                                                 <label class="switch">
                                                                      <input type="checkbox"  class="switch-class"    data-name="addon-item"  data-id="{{$item->id}}" autocomplete="off"{{ $item->active==1 ? 'checked': '' }}>
                                                                      <span class="slider round"></span>
                                                                 </label> 
                                                            </td>
                                                            <td class="td-actions text-right" style="">
                                                                 <form action="{{ route('addonitem.destroy', $item->id) }}"
                                                                      method="POST" class="form-inline float-right">
                                                                      <div class="table-icons">
                                                                           <a  title="Edit"
                                                                                class="table-action-edit"
                                                                                href="{{ route('addonitem.edit', $item->id)}}"
