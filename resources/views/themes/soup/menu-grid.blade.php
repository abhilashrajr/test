                    ...
                    <div class="col-md-10 offset-md-1" role="tablist">
                         ...
                         <div id="menu-category{{$category->id}}" class="menu-category" data-id="{{$category->id}}">
                              <div class="menu-category-title collapse-toggle" role="tab"
                                   ...
                                   aria-expanded="false">
                                   <div class="bg-image"><img src="{{ url('storage/images/'.$category->image)}}" alt="">
                                   </div>
                                   <h2 class="title">{{$category->name}}</h2>
                              </div>
                              <div id="menuContent{{$category->id}}" class="menu-category-content collapse">
                                   <div class="p-4">
                                    <div class="row gutters-sm" id="menu-item-row-{{$category->id}}">
                                   @forelse($category->menu as $menu)


                                   <div class="col-lg-4 col-6 menu-item" @if($menu->veg ==1) data-veg="true"  @endif>
                                            <!-- Menu Item -->
                                            <div class=" menu-grid-item h-100 card bg-transparent pb-4" >
                                                <img class="card-img-top mb-4" src="{{ url('storage/images/'.$menu->image)}}" alt="">
                                                <div class="card-body bg-transparent p-0">  
                                                  <h6 class="mb-0 card-title">{{$menu->name}} @if($menu->best_seller !=0)
                                                       ...
                                                       <span class="badge veg-badge-notify"
                                                            style="font-size:10px;">Veg</span>@endif</h6>
                                                  <span class="card-text text-muted text-sm" >{{$menu->description}}</span>
                                                </div>
                                                <div class="card-footer bg-transparent border-0 row  align-items-center p-0" >
                                                    <div class="col-sm-6"><span class="text-md mr-4">@if(in_array($menu->id,$addonmenu))<span class="text-muted">from</span>@endif  {{$restaurant->currency}}<span data-product-base-price>{{$menu->price}}</span></span></div>
                                                    <div class="col-sm-6 text-sm-right mt-2 mt-sm-0"><button class="btn btn-outline-secondary btn-sm add-to-cart"  data-id="{{$menu->id}}"><span>Add to cart</span></button></div>
                                                </div>
                                            </div>
                                        </div>
